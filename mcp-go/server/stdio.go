	...
	go func() {
		for {
			...
			case serverNotification := <-s.server.notifications:
				bklog.G(ctx).Debugf("💄💄 received server.notifications")
				// Only handle notifications for stdio client
				if serverNotification.Context.ClientID == "stdio" {
					bklog.G(ctx).Debugf("💄💄 avant writeResponse")
					err := s.writeResponse(
						serverNotification.Notification,
						stdout,
					)
					bklog.G(ctx).Debugf("💄💄 apres writeResponse |%+v|", err)
					if err != nil {
						s.errLogger.Printf(
	...
	for {
		...
		default:
			...
			case err := <-errChan:
				...
				return err
			case line := <-readChan:
				bklog.G(ctx).Debugf("💄💄💄 avant processMessage")
				err := s.processMessage(ctx, line, stdout)
				bklog.G(ctx).Debugf("💄💄💄 apres processMessage |%+v|", err)
				if err != nil {
					if err == io.EOF {
						return nil
					}
...
func (s *StdioServer) processMessage(
	...
	bklog.G(ctx).Debugf("💄💄💄💄 avant handleMessage")
	// Handle the message using the wrapped server
	response := s.server.HandleMessage(ctx, rawMessage)
	bklog.G(ctx).Debugf("💄💄💄💄 apres handleMessage")

	// Only write response if there is one (not for notifications)
	if response != nil {
		bklog.G(ctx).Debugf("💄💄💄💄 avant processMessage.writeResponse")
		err := s.writeResponse(response, writer)
		bklog.G(ctx).Debugf("💄💄💄💄 apres processMessage.writeResponse |%+v|", err)
		if err != nil {
