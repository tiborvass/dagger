...
func TestContainerFromLatestReleaseVersionGate(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		...
		{name: "legacy explicit opt-in", version: "v1.0.0-beta.9", ref: bare, includeSubreleases: true, want: true},
		{name: "new bare address", version: workspaceLockingVersion, ref: bare, want: true},
		{name: "explicit latest", version: workspaceLockingVersion, ref: explicitLatest},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, tc.want, containerFromUsesLatestRelease(
				ctx,
				tc.ref,
