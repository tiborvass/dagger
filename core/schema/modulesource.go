...
func (s *moduleSourceSchema) Install(dag *dagql.Server) {
	...
	dagql.Fields[*core.ModuleSource]{
		...
		dagql.NodeFunc("generateLocalDependencies", s.moduleSourceGenerateLocalDependencies).
			...
			Args(
				dagql.Arg("workspace").Doc(`The workspace to generate the local dependencies against.`),
			),
...
// transitive local dependency closure and returns the staged changes as a
// single changeset against the unstaged workspace root.
//
// Each direct local dependency is handled by (1) recursively staging the
// dependency's own transitive closure via this same field — a dagql call keyed
...
// (nested module paths, shared transitive dependencies) converges, and the
// returned changeset's before is the plain workspace root: a common base for
// whatever merge the caller does next.
//
// Remote (git) dependencies are assumed committed and are skipped. The result
// is meant to be overlaid onto the workspace (withChanges) before generating
// this module; it is not this module's own generated code.
func (s *moduleSourceSchema) moduleSourceGenerateLocalDependencies(
	ctx context.Context,
	srcInst dagql.ObjectResult[*core.ModuleSource],
	args struct {
		Workspace dagql.ID[*core.Workspace]
	...
	emptyResult := func() (dagql.ObjectResult[*core.Changeset], error) {
	...
	// walk needs — are populated. Re-resolving whenever Dependencies is empty
	// also covers a module at the workspace root (SourceRootSubpath "" or ".").
	src := srcInst
	if len(srcInst.Self().Dependencies) == 0 {
		if err := dag.Select(ctx, workspace, &src, dagql.Selector{
			Field: "moduleSource",
			Args:  []dagql.NamedInput{{Name: "path", Value: dagql.String("/" + srcInst.Self().SourceRootSubpath)}},
		}); err != nil {
			return res, fmt.Errorf("resolve module source %q dependencies: %w", srcInst.Self().SourceRootSubpath, err)
		}
	}

	// Reject dependency cycles before recursing through dagql: the recursive
	...
	// changeset's before.
	var origCtx dagql.ObjectResult[*core.Directory]
	if err := dag.Select(ctx, workspace, &origCtx, dagql.Selector{
		Field: "directory",
		Args:  []dagql.NamedInput{{Name: "path", Value: dagql.String("/")}},
	}); err != nil {
		return res, fmt.Errorf("resolve workspace root directory: %w", err)
	}

	overlay := func(base dagql.ObjectResult[*core.Directory], changes dagql.ObjectResult[*core.Changeset]) (dagql.ObjectResult[*core.Directory], error) {
		if changes.Self() == nil {
			return base, nil
		}
		...
		err = dag.Select(ctx, base, &out, dagql.Selector{
			Field: "withChanges",
...
func localDepCycle(src dagql.ObjectResult[*core.ModuleSource]) []string {
	visited := make(map[string]bool)
	onStack := make(map[string]bool)
	var visit func(s dagql.ObjectResult[*core.ModuleSource], stack []string) []string
	visit = func(s dagql.ObjectResult[*core.ModuleSource], stack []string) []string {
		key := s.Self().SourceRootSubpath
		if onStack[key] {
		...
		for _, dep := range s.Self().Dependencies {
			...
			if cycle := visit(dep, append(stack, key)); cycle != nil {
				return cycle
			}
		}
		return nil
...
func (s *moduleSourceSchema) generateOneLocalDependency(
	...
	if !ok {
		// No SDK manages this dependency: skip it rather than run every SDK's
		// generator. If the dependent then can't load it, that failure surfaces
		// downstream; this warning is the breadcrumb for a missing as-sdk entry.
		slog.Warn("generateLocalDependencies: dependency not managed by any workspace SDK; skipping",
			"dependency", depPath)
		return dagql.ObjectResult[*core.Changeset]{}, nil
	}

	wsD, err := scopedStagedWorkspace(ctx, dag, workspace, depPath, depStaging)
	if err != nil {
		return dagql.ObjectResult[*core.Changeset]{}, fmt.Errorf("scope workspace: %w", err)
	}

	// Scope generation to the owning SDK's generator (include), selected on the
	// dep-scoped workspace so the group is bound to it (BoundWorkspace) and the
	// SDK generator receives exactly the workspace the engine constructed.
	// filterGeneratorsByInclude matches the bare module name, exactly like
	// `dagger generate <sdk-name>`.
...
//
// SourceRootSubpath is treated as workspace-root-relative, which holds because
// the workspace root is the repo root by team convention; both the dedup keys
// here and the downstream changeset re-rooting rely on that.
func localDepClosureLeafFirst(src dagql.ObjectResult[*core.ModuleSource]) []dagql.ObjectResult[*core.ModuleSource] {
	return leafFirstClosure(
		src,
		func(s dagql.ObjectResult[*core.ModuleSource]) string { return s.Self().SourceRootSubpath },
		func(s dagql.ObjectResult[*core.ModuleSource]) []dagql.ObjectResult[*core.ModuleSource] {
			deps := make([]dagql.ObjectResult[*core.ModuleSource], 0, len(s.Self().Dependencies))
			for _, dep := range s.Self().Dependencies {
				// Skip only remote (git) dependencies — they are committed and
				// pinned. In-workspace deps may be LOCAL_SOURCE (host-loaded) or
				// DIR_SOURCE (resolved from the workspace root directory), and
...
// leafFirstClosure returns the transitive closure of root's children (via the
// children func), excluding root itself, in leaf-first order and deduplicated by
// key. Seeding root's key guards against a child that cycles back to the
// receiver re-adding it.
func leafFirstClosure[N any, K comparable](
	root N,
	key func(N) K,
	children func(N) []N,
) []N {
	visited := map[K]bool{key(root): true}
	var result []N
	var visit func(n N)
	visit = func(n N) {
...
// sdkOwnersByModulePath maps each workspace-managed module's cleaned
// workspace-relative path to the name of the SDK module that manages it, read
// from [[modules.<name>.as-sdk.modules]] in the workspace config. The name is
// the generator include selector (e.g. "dagger-go-sdk").
func sdkOwnersByModulePath(ctx context.Context, ws *core.Workspace) (map[string]string, error) {
...
// scopedStagedWorkspace clones base, points its cwd at the dependency, and (when
// acc carries changes) overlays the accumulated dependency changes so the
// dependency's SDK generator sees the already-generated dependencies.
func scopedStagedWorkspace(
	ctx context.Context,
	dag *dagql.Server,
	...
	// Build through the withWorkdir/withChanges fields (not NewObjectResultForCurrentCall)
