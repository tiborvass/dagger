...
func TestResolveRemoteSourceURLCachesPerSession(t *testing.T) {
	...
	resolved, err := resolveRemoteSourceURL(ctxA, ref)
	require.NoError(t, err)
	...
	resolved, err = resolveRemoteSourceURL(ctxB, ref)
	require.NoError(t, err)
	require.Equal(t, "https://github.com/dagger/dagger", resolved)
...
func TestResolveRemoteSourceURLRequiresSessionInfrastructure(t *testing.T) {
	...
	for name, ctx := range map[string]context.Context{
		...
		t.Run(name, func(t *testing.T) {
			resolved, err := resolveRemoteSourceURL(ctx, ref)
			require.NoError(t, err)
...
func TestResolveRemoteSourceURLUsesWorkspaceLock(t *testing.T) {
	var requests atomic.Int32
	srv := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requests.Add(1)
		http.Redirect(w, r, "https://github.com/dagger/dagger?dagger-get=1", http.StatusTemporaryRedirect)
	...
	resolved, err := resolveRemoteSourceURL(ctx, ref+"@main")
	require.NoError(t, err)
...
func TestResolveRemoteSourceURLWritesWorkspaceLock(t *testing.T) {
	var requests atomic.Int32
	srv := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requests.Add(1)
		http.Redirect(w, r, "https://github.com/dagger/dagger?dagger-get=1", http.StatusTemporaryRedirect)
