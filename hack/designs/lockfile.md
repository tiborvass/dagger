...
["","oci-latest",["registry.example/acme/image"],"2.0.0",[["includePrereleases",true],["protocol","http"]]]
```
...
| --- | --- | --- | --- |
| `oci-latest` | image name | `includePrereleases`, registry transport | tag |
| `oci-sha` | tagged image reference | registry transport | OCI digest |
| `git-latest` | remote URL | `includePrereleases`, `tagPrefix` | tag ref or canonical remote HEAD branch ref |
...
resolution, err := resolveLookupFromLoadedLock(lookupLock, operation, inputs, workspace.PolicyPin)
```
