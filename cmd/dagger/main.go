...
func wrapReader(r io.Reader, envName string) io.Reader {
...
func wrapWriter(w io.Writer, envName string) io.Writer {
	f := logFileFromEnv(envName)
	if f == nil {
		return w
	}
...
func init() {
	...
	stdin = wrapReader(os.Stdin, "DAGGER_LOG_STDIN")
	stdout = wrapWriter(os.Stdout, "DAGGER_LOG_STDOUT")
	stderr = wrapWriter(os.Stderr, "DAGGER_LOG_STDERR")
